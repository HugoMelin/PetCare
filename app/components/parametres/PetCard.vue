...
<template>
    ...
    <div class="flex items-center gap-3">
      ...
      <div>
        ...
        <p class="text-sm text-gray-500">
          Né le {{ new Date(pet.birthdate).toLocaleDateString() }}
        </p>
      </div>
    </div>
  ...
  <form
    v-else
    class="border border-primary rounded-lg p-4 space-y-4 bg-[#269394]/5"
    @submit.prevent="handleUpdate"
  >
    <div class="grid sm:grid-cols-2 gap-4">
      <div>
        <label class="block text-gray-700 mb-2 text-sm"> Nom </label>
        <input
          v-model="form.name"
          type="text"
          required
      ...
      <div>
        <label class="block text-gray-700 mb-2 text-sm"> Race </label>
        <input
          v-model="form.breed"
          type="text"
          class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent outline-none transition-shadow"
      ...
      <div>
        <label class="hidden sm:block text-[#269394]/0 mb-1 text-sm"
          >Propriétaires</label
        >
        <OwnerDropdown
          :pet-id="pet.id"
          :user="user"
          :pet-creator="pet.createdByUserId"
        />
      </div>
    </div>
