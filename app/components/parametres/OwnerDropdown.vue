...
console.log("OwnerDropdown petId:", petId.value);
console.log("OwnerDropdown petCreator:", petCreator.value);

const { fetchOwners } = petStore;
...
const handleAddOwner = async () => {
  ...
  try {
    const newOwner = await addPetOwner(petId.value, newOwnerEmail.value);
...
onMounted(async () => {
  try {
    const response = await fetchOwners(petId.value);
