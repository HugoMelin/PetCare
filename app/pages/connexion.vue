...
const handleSignIn = async () => {
  const { error: signInError } = await signIn(formSignIn.email, formSignIn.password, formSignIn.rememberMe);
  if (signInError) {
    error.value = signInError.message || "Une erreur est survenue lors de la connexion.";
  } else {
    error.value = null;
  }
};
    ...
    <Card>
      ...
      <template #content>
        ...
        >
          ...
          <p v-if="error && error === 'Invalid email or password'" class="mt-4 text-red-600 text-sm">
            Email ou mot de passe incorrect.
          </p>
          <p v-else-if="error && error !== 'Invalid email'" class="mt-4 text-red-600 text-sm">
            {{ error }}
          </p>
        </form>
        ...
        <div class="mt-6 text-center">
          <p class="text-gray-600">
            ...
            <RouterLink to="/inscription">
              <span class="text-primary hover:text-primary/90">Créer un compte</span>
            </RouterLink>
          </p>
        </div>
